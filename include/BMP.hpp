...
struct BMP {
    ...
    std::vector<uint16_t> get_pixel(uint32_t x0, uint32_t y0){
        if (x0 >= (uint32_t)bmp_info_header.width || y0 >= (uint32_t)bmp_info_header.height || x0 < 0 || y0 < 0) {
            throw std::runtime_error("The point is outside the image boundaries!");
        }
        ...
        uint32_t channels = bmp_info_header.bit_count / 8;
        RGBA.push_back(data[channels * (y0 * bmp_info_header.width + x0) + 2]); //= R;
        RGBA.push_back(data[channels * (y0 * bmp_info_header.width + x0) + 1]); //= G;
        RGBA.push_back(data[channels * (y0 * bmp_info_header.width + x0) + 0]); //= B;
        if (channels == 4) {
            RGBA.push_back(data[channels * (y0 * bmp_info_header.width + x0) + 3]); //= A;
        }
